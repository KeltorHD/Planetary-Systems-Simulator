...
void Gui::updateInput(sf::RenderWindow* window, std::queue<sf::Event>& events)
{
	...
	else if (isLeftPress) /*открытие меню изменени€ добавленного объекта*/
	{
		bool isFind{ false };
...
void Gui::update()
{
	...
	/*отрисовка меню добавлени€ объекта*/
	this->updateAddObj();

	if (this->add_obj) /*обновление добавл€емого объекта*/
		add_obj->update();
}
		...
		if (ImGui::TreeNode(this->locale->get_c("objects")))
		{
			...
			for (size_t i = 0; i < this->simulation->getCountObj(); i++)
			{
				...
				if (ImGui::TreeNode(std::string(this->locale->get_s("object") + " " + std::to_string(i + 1)).c_str()))
				{
					...
					if (ImGui::InputText(this->locale->get_c("object_name"), this->edit_name_obj, 256)) /*им€ тела*/
					{
